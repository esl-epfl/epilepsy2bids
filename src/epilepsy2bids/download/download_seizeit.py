...
def downloadSeizeit(destination: str, apiToken: str) -> None:
    ...
    logging.basicConfig(filename=os.path.join(destination, 'downloadScript.log'),
                        filemode='a',
    ...
    if respFileList.ok:
        ...
        for file in fileList['data']:
            ...
            Path(os.path.join(destination, file['directoryLabel'])).mkdir(parents=True, exist_ok=True)
            filename = os.path.join(destination, file['directoryLabel'], file['dataFile']['filename'])
            ...
            if resp.ok:
                ...
                logging.error('{} - Failed to download file : {}'.format(file['dataFile']['id'], resp.status_code))
    else:
        logging.error('Failed to retrieve dataset list status : {}'.format(respFileList.status_code))
